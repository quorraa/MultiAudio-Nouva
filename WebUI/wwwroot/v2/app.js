...
function renderStrip(o) {
  ...
  return `<div class="ch-strip" data-slot="${o.slotIndex}" style="--ch-color:${color}">
    <div class="ch-head">
    ...
    <div class="ch-meter">
      <div class="ch-meter-bar"><div class="ch-meter-fill" data-meter="${o.slotIndex}" style="width:${Math.round((o.meterLevel||0)*100)}%"></div></div>
      <div class="ch-meter-readout"><span data-pct="${o.slotIndex}">${Math.round((o.meterLevel||0)*100)}%</span><span>${esc(o.syncLockState)}</span></div>
    </div>
...
function patchTele(f) {
  ...
  for (const t of f.outputs || []) {
    ...
    if (fill) fill.style.width = `${Math.round((t.meterLevel||0)*100)}%`;
    if (pctEl) pctEl.textContent = `${Math.round((t.meterLevel||0)*100)}%`;
